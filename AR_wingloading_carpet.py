import numpy as np
import atmosprops,math # type: ignore
import matplotlib.pyplot as plt
import plotly.graph_objects as go

 
'''
 
Analyze how AR and W/S effect the T/W loading
Analyze how'''

M = 0.9
h = 45000
q = atmosprops.imperial_atmosphere(h).density() * ((atmosprops.imperial_atmosphere(h).speed_of_sound()*M)**2)
q = float(q)

W_S = 100
T_W = []
AR = [4,6,8,10,12,14]
W_S = [50,75,100,125,150]
e = 0.8
T_W1 = []
CDo = 0.02
for i in W_S:
    for j in AR:
        k = 1/(math.pi*e*j)
        T_W1.append(((q*CDo)/(i)) + ((i*k)/q))
        print(f"W/S={i}, AR={j}, T/W={(((q*CDo)/(i)) + ((i*k)/q))}")
    T_W.append(T_W1)
    T_W1 = []



fig = go.Figure(go.Carpet(a=AR, b=W_S, y=T_W, aaxis=dict(tickprefix = 'AR=',gridcolor='black', linecolor='black', smoothing=1.3),
    baxis=dict(tickprefix = 'W/S=',gridcolor='black', linecolor='black', smoothing=1.3)))

fig.update_layout(
    title_text="T/W Sensitivity with CDo and e (oswalds efficiency)", # Use title_text or title=dict(text="...")
    title_x=0.5, # Centers the title above the plot
    title_font=dict(size=24, family="Arial", color="black") # Optional: Customize font
)

fig.show()

